 ...
 */
package com.sap.cd.athenas.cockpits.setup;
...
import com.sap.cd.athenas.cockpits.constants.AthenasCockpitsConstants;
 ...
 */
@SystemSetup(extension = AthenasCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(AthenasCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(AthenasCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						AthenasCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/athenascockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
